module bullet2.LinearMath.btScalar;
...
version (BT_USE_DOUBLE_PRECISION)
{
	...
	enum BT_LARGE_FLOAT = 1e30;
}
else
{
	alias btScalar = float;
	//keep BT_LARGE_FLOAT*BT_LARGE_FLOAT < FLT_MAX
    enum BT_LARGE_FLOAT = 1e18f;
}
...
enum SIMD_PI = btScalar(3.1415926535897932384626433832795029);
enum SIMD_2_PI = (btScalar(2.0) * SIMD_PI);
enum SIMD_HALF_PI = (SIMD_PI * btScalar(0.5));
enum SIMD_RADS_PER_DEG = (SIMD_2_PI / btScalar(360.0));
enum SIMD_DEGS_PER_RAD = (btScalar(360.0) / SIMD_2_PI);
enum SIMDSQRT12 = btScalar(0.7071067811865475244008443621048490);

btScalar btRecipSqrt(T)(auto ref T x)
{
	return cast(btScalar)(btScalar(1.0) / btSqrt(btScalar(x))); /* reciprocal square root */
}
btScalar btRecip(T)(auto ref T x)
{
	return btScalar(1.0) / btScalar(x);
}

version (BT_USE_DOUBLE_PRECISION)
{
	enum SIMD_EPSILON = double.epsilon;
	enum SIMD_INFINITY = double.max;
	enum BT_ONE = 1.0;
	enum BT_ZERO = 0.0;
	enum BT_TWO = 2.0;
	enum BT_HALF = 0.5;
...
{
	enum SIMD_EPSILON = float.epsilon;
	enum SIMD_INFINITY = float.max;
	enum BT_ONE = 1.0f;
	enum BT_ZERO = 0.0f;
	enum BT_TWO = 2.0f;
	enum BT_HALF = 0.5f;
...
//#endif
//alias btFsels = btFsel;
btScalar btFsels(btScalar a, btScalar b, btScalar c)
{
	return btFsel(a, b, c);
}
...
/*SIMD_FORCE_INLINE*/ uint btSelect(uint condition, uint valueIfConditionNonZero, uint valueIfConditionZero)
{
...
/*SIMD_FORCE_INLINE*/ int btSelect(uint condition, int valueIfConditionNonZero, int valueIfConditionZero)
{
	uint testNz = cast(uint)((cast(int)condition | -cast(int)condition) >> 31);
	uint testEqz = ~testNz;
	return cast(int)((valueIfConditionNonZero & testNz) | (valueIfConditionZero & testEqz));
}
/*SIMD_FORCE_INLINE*/ float btSelect(uint condition, float valueIfConditionNonZero, float valueIfConditionZero)
{
...
/*SIMD_FORCE_INLINE*/ uint btSwapEndian(uint val)
{
	return (((val & 0xff000000) >> 24) | ((val & 0x00ff0000) >> 8) | ((val & 0x0000ff00) << 8) | ((val & 0x000000ff) << 24));
}

/*SIMD_FORCE_INLINE*/ ushort btSwapEndian(ushort val)
{
	return cast(ushort)(((val & 0xff00) >> 8) | ((val & 0x00ff) << 8));
}

/+/*SIMD_FORCE_INLINE*/ uint btSwapEndian(int val)
{
	return btSwapEndian(cast(uint)val);
}

/*SIMD_FORCE_INLINE*/ ushort btSwapEndian(short val)
{
	return btSwapEndian(cast(ushort)val);
}+/

/*SIMD_FORCE_INLINE*/ int btSwapEndian(int val)
{
	return cast(int)btSwapEndian(cast(uint)val);
}

/*SIMD_FORCE_INLINE*/ short btSwapEndian(short val)
{
	return cast(short)btSwapEndian(cast(ushort)val);
}
/+
...
//template <typename T>
T* btAlignPointer(T)(T* unalignedPtr, size_t alignment)
{
